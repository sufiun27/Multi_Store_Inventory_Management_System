    ...
    <main>
        <div class="container-fluid px-4">
            <table class="table table-striped">
                ...
                $sql = "SELECT TOP (50) iss.is_po_no, iss.is_id, i.i_name,i.i_price, c.c_name, 
                ...
                iss.is_datetime, iss.is_item_issue_by, e.e_com_id, e.e_name
                     ...
                     INNER JOIN (
					 SELECT SUM(ist_qty*ist_price) as total_price, is_id,  SUM(ist_qty*ist_price)/SUM(ist_qty) as is_avg_price
					 FROM item_issue_trac GROUP BY is_id) ist ON ist.is_id = iss.is_id
                     WHERE iss.is_active = 1 and i.section = '{$section}'
                     ORDER BY iss.is_datetime DESC";
                }
                if($_SESSION['table']=='all'){
                    $sql = "SELECT iss.is_po_no, iss.is_id, i.i_name,i.i_price, c.c_name, 
                    i.i_manufactured_by, i.i_size, i.i_unit, ist.is_avg_price, 
                    iss.is_qty, ist.total_price as total_price, 
                    iss.is_datetime, iss.is_item_issue_by, e.e_com_id, e.e_name
                         from item_issue iss 
                ...
                if ($stmt->execute()) {
                   // $product = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    while ($product = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>".$product['i_price']."</td>";//sys price
